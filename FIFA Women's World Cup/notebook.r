...
summary(wwc_raw)
str(wwc_raw)
#Attendance got NAs, change date/day/wk to date
...
# .... YOUR CODE FOR TASK 6 .... 
ggplot(data =wwc_3, aes(y=attendance , x=venue) ) +
geom_boxplot()+
# geom_jitter() is a convenient way to add random variation to the location of each point. 
#This random variation makes overlapping points easier to visualize.
...
wwc_3  %>% group_by(venue) %>%
summarize(n = n(), min_attendance = min(attendance),
          max_attendance = max(attendance))

# Correct the outlier
wwc_4  <- wwc_3  %>%  mutate(attendance = replace(attendance, which.max(attendance),57900))

# Print an updated summary table 
wwc_venue_summary <- wwc_4  %>% group_by(venue) %>%
summarize(n = n(), min_attendance = min(attendance),
          max_attendance = max(attendance))
print(wwc_venue_summary)
